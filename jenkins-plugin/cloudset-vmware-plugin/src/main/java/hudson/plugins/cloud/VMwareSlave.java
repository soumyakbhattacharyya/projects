...
import hudson.plugins.cloud.ssh.VMwareUnixLauncher;
 ...
 */
public final class VMwareSlave extends Slave {
	/**
	...
	public final String securityCredential;
	public final String accessId;
	public final String cloudType;
	public final String accountNumber;
	public final String userId;
	public final VMwarePrivateKey parentCloudPrivateKey;
	...
	public VMwareSlave(String remoteAdminPassword, boolean isExistingInstance,
			String accountNumber, String userId, String securityCredential,
			String accessId, String cloudType, String instanceId,
			String description, String remoteFS, int sshPort, int numExecutors,
			String labelString, String initScript, String remoteAdmin,
			String rootCommandPrefix, String jvmopts,
			VMwarePrivateKey parentCloudPrivateKey, String projectUserID,
			String projectID) throws FormException, IOException {
		this(remoteAdminPassword, isExistingInstance, accountNumber, userId,
				securityCredential, accessId, cloudType, instanceId,
				description, remoteFS, sshPort, numExecutors, Mode.NORMAL,
				labelString, initScript, Collections
						.<NodeProperty<?>> emptyList(), remoteAdmin,
				rootCommandPrefix, jvmopts, parentCloudPrivateKey,
	...
	public VMwareSlave(String remoteAdminPassword, boolean isExistingInstance,
		...
		super(instanceId, description, remoteFS, numExecutors, mode,
				labelString, new VMwareUnixLauncher(),
				new VMwareRetentionStrategy(), nodeProperties);
	...
	public VMwareSlave(String remoteAdminPassword, boolean isExistingInstance,
			String accountNumber, String userId, String securityCredential,
			String accessId, String cloudType, String instanceId,
			String projectUserID, String projectID) throws FormException,
			IOException {
		this(remoteAdminPassword, isExistingInstance, accountNumber, userId,
				securityCredential, accessId, cloudType, instanceId, "debug",
				"/tmp/hudson", 22, 1, Mode.NORMAL, "debug", "", Collections
						.<NodeProperty<?>> emptyList(), null, null, null, null,
				projectUserID, projectID);
	}
	...
	public Computer createComputer() {
		return new VMwareComputer(this);
	}

	/**
	 * CTSJPAASBAAS-5 - Terminates the instance in Eucalyptus with additional
	 * parameters
	 * 
	 * This method is responsible for terminating the instance except two cases.
	 * 1.Delete the instance from UI 2.Delete Vm due to retention Strategy
	 * 
	 * LOGIC ---> a. if VM is going to be a Type of J (JIT) It will terminate
	 * and deregister from Jenkins Tree.
	 * 
	 * b. if VM is going to be a Type of P (POLLED) It will not going to do
	 * anything
	 * 
	 * c. if VM is going to be a Type of T (Third Party) It will be only
	 * deregistered from Jenkins tree
	 * 
	 * d. if VM is going to be a Type of N (Anything else) It will not going to
	 * do anything
	 * 
	 */
	public void terminateInstance(Slave s, String... argToTerminateInstance)
			throws Exception {

		String accountNumber = this.accountNumber;
		String userId = this.userId;
		String instanceId = ((VMwareSlave) s).getInstanceId();
		...
		if (null != argToTerminateInstance
				&& argToTerminateInstance.length == 1) {
			...
			if (vmType.equalsIgnoreCase("J")) {
				...
				PlatformInfraUtil
						.terminateVirtualMachine(accountNumber, instanceId,
								userId, this.securityCredential, this.accessId,
								this.cloudType, projectUserID, projectID);
	 ...
	 * 
	 */
	public void terminateInstance(String instanceIdToBeTerminated,
			String projectUserID, String projectID,
			String... argToTerminateInstance) throws Exception {
		...
		PlatformInfraUtil.terminateVirtualMachine(accountNumber, instanceId,
				userId, this.securityCredential, this.accessId, this.cloudType,
				projectUserID, projectID);
	...
	private static final Logger LOGGER = Logger.getLogger(VMwareSlave.class
			.getName());

package hudson.plugins.cloud.ssh;

import hudson.model.Descriptor;
import hudson.model.Hudson;
import hudson.plugins.cloud.VMwareComputer;
import hudson.plugins.cloud.VMwareComputerLauncher;
import hudson.plugins.cloud.VMwareConstant;
import hudson.plugins.cloud.resource.ServiceLocator;
...
/**
 * {@link ComputerLauncher} that connects to a Unix slave on VMware by using SSH.
 * 
 * @author Soumyak 
 */
public class VMwareUnixLauncher extends VMwareComputerLauncher
{
	 ...
	 * 
	 */
	protected void launch(VMwareComputer computer, PrintStream logger, VirtualMachineType server) throws IOException, InterruptedException, S3ServiceException
	{
		try
		{
	 ...
	 */
	private void executeInitScript(VMwareComputer computer, PrintStream logger, Connection connection) throws IOException, InterruptedException, UnsupportedEncodingException
	{
	 ...
	 * @throws IOException
	 */
	private void authenticate(VMwareComputer computer, PrintStream logger, Connection connection) throws IOException
	{
		int tryCount = 1;
		String retry_count_value = ServiceLocator.getInstance()
		.getVmwareInfraVirtualmachineIpDiscoveryTime();
		int retry_count =0;
		
		if(retry_count_value != null && !retry_count_value.isEmpty()){
			retry_count = Integer.parseInt(retry_count_value.trim());
		}else{
			retry_count = VMwareConstant.RETRY_COUNT;
		}
	 ...
	 * @throws IOException
	 */
	private Connection getConnection(VMwareComputer computer, PrintStream logger) throws IOException
	{

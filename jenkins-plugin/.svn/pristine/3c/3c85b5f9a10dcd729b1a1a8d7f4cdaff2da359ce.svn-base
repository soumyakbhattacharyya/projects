...
public abstract class EC2Cloud extends Cloud {
	 ...
	 */
	public int countCurrentEC2Slaves(ResourceOptions resourceOptions)
			throws CloudException {
	
		return PlatformInfraUtil.getVirtualMachineListCount(resourceOptions);
	}
	...
	/**
	 * Provision a new VM for an ami from UI
	 * 
	 * @param req
	 * @param rsp
	 * @param ami
	...
	private EC2Slave getSlaveFromUI(StreamTaskListener listener, Object obj,
		...
		if (obj instanceof SlaveTemplate) {
			...
			resourceOptions.setAccountNumber(getAccountId());
			resourceOptions.setPublicCredential(accessId);
			resourceOptions.setPrivateCredential(secretKey.getPlainText());
			resourceOptions.setPrivateKey(privateKey);
			
			resourceOptions.setVmLoginUserID(getVmOwnerId());
			resourceOptions.setImageId(t.ami);
			resourceOptions.setKernelID(null);
			resourceOptions.setImageType(t.type.getTypeId());
			resourceOptions.setMaxInstance(1);
			resourceOptions.setMinInstance(1);
			resourceOptions.setKey(getSecurityKeyFileName());
			resourceOptions.setSecurityGroup(getSecurityGroup());			
			resourceOptions.setCloudType(t.parent.name);
			resourceOptions.setDataCenterId(t.getAvailableZone());
			resourceOptions.setSubnetID(t.getSubnetID());
			resourceOptions.setEndPoint(endPoint);
						
			resourceOptions.setProjectUserId(projectUserID);
			resourceOptions.setProjectID(projectID);
			
			s = t.provisionFromUI(listener,resourceOptions);
	...
	private void getVMPolicy(final Label label, int excessWorkload,
		...
		final String availableZone = t.availableZone;
		final String subnetID = t.subnetID;
		
			
		final ResourceOptions resourceOptions = new ResourceOptions();
		...
		resourceOptions.setKey(key);
		resourceOptions.setVmLoginUserID(userId);
		resourceOptions.setDataCenterId(availableZone);
		resourceOptions.setSubnetID(subnetID);
		resourceOptions.setProjectID(projectUserID);
		resourceOptions.setProjectID(projectID);
		resourceOptions.setProviderID("02");
		resourceOptions.setStartStopMachine(t.startStopSlave);
		resourceOptions.setSlaveLabel(label.getDisplayName());
		resourceOptions.setCloudType(cloudType);
		resourceOptions.setImageId(imageId);
		resourceOptions.setImageType(imageType);
		resourceOptions.setKernelID(kernelId);
		resourceOptions.setMaxInstance(maxInstance);
		resourceOptions.setMinInstance(minInstance);
		resourceOptions.setSecurityGroup(group);
		
		

		for (; excessWorkload > 0; excessWorkload--) {
			if (countCurrentEC2Slaves(resourceOptions) >= instanceCap) {
					...
					Computer.threadPoolForRemoting.submit(new Callable<Node>() {
						public Node call() throws Exception {
							...
							if (StringUtils.startsWithIgnoreCase(
								 ...
								 * Terminated
								 **/
								// set it to false,its only applicable for EXT# machines
								resourceOptions.setStartStopMachine(false);
								
								s = t.provision(new StreamTaskListener(
										System.out), resourceOptions);
							} else {								
								// This is a pooled instance, we support start stop only for third party
								resourceOptions.setStartStopMachine(false);
	...
	public static abstract class DescriptorImpl extends Descriptor<Cloud> {
		 ...
		 * @throws ServletException
		 */
		protected FormValidation doTestConnection(ResourceOptions resourceOptions) throws IOException,
				ServletException {
			try {
				...
				int cntRunningInst = PlatformInfraUtil
						.getVirtualMachineListCount(resourceOptions);
		...
		/** 
		 * Not Supporting Generate key option
		 * 
		 * @param rsp
		 * @param ec2EndpointUrl
		 * @param accessId
		 * @param secretKey
		 * @return
		 * @throws IOException

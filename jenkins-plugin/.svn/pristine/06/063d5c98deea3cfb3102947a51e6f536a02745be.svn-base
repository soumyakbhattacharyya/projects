...
public class PlatformInfraUtil {
	...
	public static boolean isRunningVirtualMachine(String accountNumber,
			...
			String publicCredential, String providerId, String projectUserId,
			String projectId) {
		...
		providerContext = getCloudProviderContext(providerId, providerContext);
	...
	public static int getVirtualMachineListCount(String accountNumber,
			...
			String providerId, String projectUserId, String projectId) {
		...
		addressing.setValue(ServiceLocator.getInstance().getVMwareInfraAddressing());
	...
	public static String launchVirtualMachine(String accountNumber,
			...
			int minInstance, int maxInstance,
		...
		if (!providerId.equalsIgnoreCase(ClientConstants.VMWARE_PROVIDER)) {
			...
			addressing.setKey("addressing");
			addressing
					.setValue(ServiceLocator.getInstance().getVMwareInfraAddressing());
			propertiesList.add(addressing);
			providerContext.setCustomProperties(propertiesList);
		}
		...
		context = populateContextForLaunch(context, imageId, imageType, 
				providerId);
	...
	private static RequestContext populateContextForLaunch(
		...
		if (providerId.equalsIgnoreCase(ClientConstants.VMWARE_PROVIDER)) {
			...
			productType.setName("test-vm-launch");
			machineResource.setName("test-vm-launch");
		}

		if (providerId.equalsIgnoreCase(ClientConstants.VMWARE_PROVIDER)) {
			productType = populateProductTypeForVMWare(imageType, productType);
		}
	...
	private static ProviderContext getCloudProviderContext(String providerId,
		...
		if (providerId.equalsIgnoreCase(ClientConstants.VMWARE_PROVIDER)) {
			providerContext.setCloudName(ClientConstants.VMWARE_PROVIDER_NAME);
			providerContext.setProviderName(ClientConstants.VMWARE_PROVIDER_VALUE);

...
public class EC2Computer extends SlaveComputer {
	...
	public InstanceState getState() {
		...
		String ec2endpoint = this.getNode().ec2endpoint;
		...
		resourceOptions.setPrivateCredential(this.getNode().secretKey);
		resourceOptions.setCloudType(cloudType);
		resourceOptions.setEndPoint(ec2endpoint);
		resourceOptions.setProjectUserId(projectUserID);
		resourceOptions.setProjectID(projectID);
		
		if(this.getNode().getAvailableZone()!= null && StringUtils.isBlank(this.getNode().getAvailableZone())){
			resourceOptions.setDataCenterId(this.getNode().getAvailableZone());
		}
		
		if(this.getNode().getSubnetID()!= null && StringUtils.isBlank(this.getNode().getSubnetID())){
			resourceOptions.setSubnetID(this.getNode().getSubnetID());
		}
		
		return InstanceState.find(PlatformInfraUtil.
				getVirtualMachineDetails(instanceId, 
						resourceOptions));
	...
	public boolean isInRunningState(String accountNumber, String instanceId,
			String userId, String secretKey, String accessId, String cloudType,String endPoint,String projectUserID,String projectID) {
		// Is this instance already registered for the given credential
		ResourceOptions resourceOptions = new ResourceOptions();
		resourceOptions.setAccountNumber(accountNumber);
		resourceOptions.setVmLoginUserID(userId);
		resourceOptions.setPublicCredential(accessId);
		resourceOptions.setPrivateCredential(secretKey);
		resourceOptions.setCloudType(cloudType);
		...
		boolean flag = PlatformInfraUtil.isRunningVirtualMachine(instanceId, resourceOptions);
	...
	private Server _describeInstance() {
		...
		instanceId = removeThirdPartyConstant(instanceId);
		ResourceOptions resourceOptions = new ResourceOptions();
		resourceOptions.setAccountNumber(this.getNode().accountNumber);
		resourceOptions.setVmLoginUserID(this.getNode().userId);
		resourceOptions.setPublicCredential(this.getNode().accessId);
		resourceOptions.setPrivateCredential(this.getNode().secretKey);
		resourceOptions.setCloudType(this.getNode().cloudType);
		resourceOptions.setEndPoint(this.getNode().ec2endpoint);
		resourceOptions.setProjectUserId(this.getNode().getProjectUserID());
		resourceOptions.setProjectID(this.getNode().getProjectID());
		if(this.getNode().getAvailableZone()!= null && StringUtils.isBlank(this.getNode().getAvailableZone())){
			resourceOptions.setDataCenterId(this.getNode().getAvailableZone());
		}
		...
		VirtualMachineType machineType = PlatformInfraUtil.getVirtualMachine(instanceId, resourceOptions);

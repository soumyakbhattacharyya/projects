...
public class PlatformInfraUtil {
	...
	private static ProviderContext getCloudProviderContext(String providerId,
		...
		if (providerId.equalsIgnoreCase(ClientConstants.EUCA_PROVIDER)) {
			providerContext.setCloudName(ClientConstants.EUCALYPRUS_CLOUD);
			providerContext.setProviderName(ClientConstants.EUCALYPTUS_PROVIDER_VALUE);
			providerContext.setEndpoint(ServiceLocator.getInstance()
					.getEucalyptusUrl());
		} else if (providerId.equalsIgnoreCase(ClientConstants.EC2_PROVIDER)) {
			...
			providerContext.setEndpoint(ServiceLocator.getInstance()
					.getAwsUrl());
		} else if (providerId.equalsIgnoreCase(ClientConstants.VMWARE_PROVIDER)) {

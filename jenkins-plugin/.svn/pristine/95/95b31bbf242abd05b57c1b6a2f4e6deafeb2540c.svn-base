 ...
 * FileUtil
 *
 * consists of all file / folder related utility methods
 *
 * @author cognizant
 *
 */
...
//	}
	...
	/**
	 * <p>For each file in the source folder <ul> <li>if file is not in destination, this is a new file</li> <li>if the destination file
	 * exists but is old, this is a modified file</li> </ul>
	 *
	 * <p>Note: the time parameter (1st param) is basically not used in the code. On Windows, the lastModifiedDate will not be updated when
	 * you copy a file to the source folder, until we have a way to get the "real" lastModifiedDate on Windows, we won't use this "time"
	 * field</p>
	 *
	 * @param time should be the last build time, to improve performance, we will list all files modified after "time" and check with
	 * destination
...
//		}
		...
		while (it.hasNext()) {
			...
			try {
				...
				if (!tmp.exists()) {
					if (!relativeName.contains(Constants.LAST_PREFLIGHT_TIMESTAMP + Constants.TEXT_FILE_EXTENTION)) {
						newOrModified = true;
						list.add(new Entry(relativeName, Entry.Type.NEW));
						LogUtil.log("New file: " + relativeName);
					}
					...
					if (!relativeName.contains(Constants.LAST_PREFLIGHT_TIMESTAMP + Constants.TEXT_FILE_EXTENTION)) {
						...
						LogUtil.log("Modified file: " + relativeName);
					}
				}
			} catch (IOException e) {
				LogUtil.log(e);
	...
	/**
	 * <p>For each file in the destination folder <ul> <li>if file is not in source, and it is in the allowDeleteList, this file will be
	 * deleted in the destination</li> </ul>
	 *
	 * <p>Note: the time parameter (1st param) is basically not used in the code. On Windows, the lastModifiedDate will not be updated when
	 * you copy a file to the source folder, until we have a way to get the "real" lastModifiedDate on Windows, we won't use this "time"
	 * field</p>
...
//		}
		...
		while (it.hasNext()) {
			...
			try {
				...
				if (!allSources.contains(tmp) && (null == allowDeleteList || allowDeleteList.contains(relativeName))) {
					if (!relativeName.startsWith(Constants.DOT_GIT_REPOSITORY)) {
						list.add(new Entry(relativeName, Entry.Type.DELETED));
						LogUtil.log("File to be deleted to sync developer workspace and local: " + relativeName);
					}
				}
			} catch (IOException e) {
	...
	/**
	 * Get the relative path of fileName and folderName <ul> <li>fileName = c:\abc\def\foo.java</li> <li>folderName = c:\abc</li>
	 * <li>relativeName = def\foo.java </ul> This function will not handle Unix/Windows path separator conversation, but will append a
	 * java.io.File.separator if folderName does not end with one
	 *
	...
	// extract the time stamp from the file
	public long getLastBuildTimestamp(final File lastPreflightTimestampTextFile) throws IOException {
		long lastPreflightBuildTime = 0;
		String lastPreflightBuildTimeStringified = FileUtils.readFileToString(lastPreflightTimestampTextFile);
		if (null != lastPreflightBuildTimeStringified && lastPreflightBuildTimeStringified.contains("=")) {
			String[] intermediate = lastPreflightBuildTimeStringified.split("=");
			// 2nd element should be the last time stamp
			String lstTmStmp = intermediate[1];
			lastPreflightBuildTime = Long.parseLong(lstTmStmp);
		} else {
			throw new AssertionError("last build time stamp not available");
		}
		return lastPreflightBuildTime;
	}

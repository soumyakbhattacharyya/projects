...
public final class S3BucketPublisher extends Recorder implements Describable<Publisher> {
    ...
    public List<Entry> getEntries() {
    	System.out.println("List size isf "+entries.size());
        return entries;
    }
    ...
    public static final class DescriptorImpl extends BuildStepDescriptor<Publisher> {
        ...
        public S3Profile[] getProfileForProject() {
        	if(!ObjectUtils.equals(profiles,null)){
        		LOGGER.info("Filtering profile based on project id for all profiles having size of "+profiles.size());
        		CopyOnWriteList<S3Profile> filteredProfiles = new CopyOnWriteList<S3Profile>();
        		String projectId = "";
        		StaplerRequest req = Stapler.getCurrentRequest();
        		if(!ObjectUtils.equals(req, null)){
        			projectId  = String.valueOf(req.getSession(false).getAttribute(PROJECT_ID));
        		}        		        		
            	for(S3Profile s3 : profiles){
            		if(StringUtils.contains(s3.getName(),projectId)){
            			filteredProfiles.add(s3);
            		}
            	}
                return filteredProfiles.toArray(new S3Profile[0]);
        	}
        	else{
        		// Stay ASIS : When no profile has been created
        		LOGGER.info("No S3 profile has been created yet");
        		return profiles.toArray(new S3Profile[0]);

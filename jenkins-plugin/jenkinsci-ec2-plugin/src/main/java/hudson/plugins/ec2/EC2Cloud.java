...
public abstract class EC2Cloud extends Cloud {
	...
	/**
	 * Extract the VMWare specific templates We first get the list of the cloud
	 * available in the Jenkins instance and then cloud objects and then return
	 * the list templates
	 * 
	...
	private List<VMWareSlaveTemplate> extractVMWareTemplates() {
		...
		for (Cloud c : cl) {
			if (c != null && c instanceof VMWareCloud) {
				...
				// if(this.getAccessId().equals(cloud.getAccessId()) &&
				// this.getAccountId().equals(cloud.getAccountId())){
				...
				/*
				 * Object newObject = null; try{ Method method
				 * =ReflectionUtils.findMethod(c.getClass(),
				 * EC2Constant.VMWARETEMPLATES); newObject =
				 * ReflectionUtils.invokeMethod(method, c); } catch(Exception
				 * ex){ ex.printStackTrace(); } if (newObject instanceof List) {
				 * return (List<VMWareSlaveTemplate>) newObject;
				 * 
				 * }
				 */
			}
	...
	public Object getTemplate(String ami) {
		...
		if (vmwareTemplateList != null) {
			for (VMWareSlaveTemplate vmWareSlaveTemplate : vmwareTemplateList) {
				...
				String labelString = vmWareSlaveTemplate.getLabelString();
				if (templateAmi.equals(ami)
						&& (!(labelString.contains(EC2Constant.THIRDPARTY)) && (!labelString
								.contains(EC2Constant.JIT)))) {
	...
	public Object getTemplate(Label label) {
		...
		if (vmwareTemplateList != null) {
			for (VMWareSlaveTemplate vmWareSlaveTemplate : vmwareTemplateList) {
				if (label == null
						|| label.matches(vmWareSlaveTemplate.getLabelSet())) {
					return vmWareSlaveTemplate;
				}
			}
		}
	...
	public Object getTemplate(Label label, Cloud c) {
		if (c instanceof AmazonEC2Cloud || c instanceof Eucalyptus) {
			for (SlaveTemplate t : templates)
				if (label == null || label.matches(t.getLabelSet()))
					return t;
		} else if (c instanceof VMWareCloud) {
			List<VMWareSlaveTemplate> vmwareTemplateList = extractVMWareTemplates();
	...
	public void doProvision(StaplerRequest req, StaplerResponse rsp,
		...
		if (req != null
				&& req.getSession() != null
				&& req.getSession().getAttribute(ClientConstants.JPAAS_USER) != null) {
			try {
				JPaaSUser juser = (JPaaSUser) req.getSession().getAttribute(
						ClientConstants.JPAAS_USER);
	...
	private String getCloudType(String cloudName) {
		...
		case eucalyptus:
			return EC2Constant.EUCACloud;
		case amazon:
			return EC2Constant.EC2Cloud;
		case vmware:
			return EC2Constant.VMWareCloud;
		default:
	...
	private EC2Slave getSlaveFromUI(StreamTaskListener listener, Object obj,
			...
			String userIdForUser, String projectUserID, String projectID)
		...
		if (obj instanceof SlaveTemplate) {
			...
			s = t.provisionFromUI(listener, accountNumber, userId, imageId,
					...
					t.labels, projectUserID, projectID);

		} else if (obj instanceof VMWareSlaveTemplate) {
			...
			String imageId = t.ami;
			String kernelId = null;
			final String imageType = t.cpuType.getTypeId() + ":"
					+ t.ramType.getTypeId();
			// String imageType = t.type.getTypeId();
			int minInstance = 1;
			int maxInstance = 1;
		...
		try {
			...
			if (obj instanceof SlaveTemplate) {
				...
				for (; excessWorkload > 0; excessWorkload--) {
					 ...
					 * "Instance cap reached, not provisioning."); break; //
					 * maxed out }
					 */
			...
			return r;
		} catch (/* EC2Exception */Exception e) {
	 ...
	 * @return
	 */
	public Collection<PlannedNode> provisionForProject(final Label label,
			int excessWorkload, String... projectSpecificParameter) {
		try {
			String projectUserID = "";
			String projectID = "";

			if (null != projectSpecificParameter
					&& projectSpecificParameter.length > 0) {
				...
				LOGGER.info("projectSpecificParameter account id"
						+ projectSpecificParameter[0]);
				try {
					projectUserID = projectSpecificParameter[5];
					projectID = projectSpecificParameter[4];
					LOGGER.info("projectUserid and projectId is "
			...
			if (obj instanceof SlaveTemplate) {
				...
				final VMWareSlaveTemplate t = (VMWareSlaveTemplate) obj;
				getVMWareVMPolicy(label, excessWorkload, r, t, projectUserID,
						projectID);
	...
	private void getVMPolicy(final Label label, int excessWorkload,
		...
		for (; excessWorkload > 0; excessWorkload--) {
			if (countCurrentEC2Slaves(accountNumber, userId, cloudType,
						...
						public Node call() throws Exception {
							...
							if (StringUtils.startsWithIgnoreCase(
								...
								s = t.findToRegisterExistingInstance(
										...
										cloudType, privateKey, projectUserID,
										projectID);
	 ...
	 * @throws CloudException
	 */
	private void getVMWareVMPolicy(final Label label, int excessWorkload,
			List<PlannedNode> r, final VMWareSlaveTemplate t,
			final String projectUserID, final String projectID)
			throws CloudException {
		// LOGGER.info(" Account "+getAccountId()+" VM Owner "+getVmOwnerId()+" Sec Key "+getSecretKey()+" Sec Grp "+getSecurityGroup());
		...
		protected FormValidation doTestConnection(URL ec2endpoint,
			...
			try {
				...
				if (null == ec2endpoint) {
					...
					LOGGER.info("Testing connection for Eucalyptus cloud");
					providerCode = EC2Constant.EUCACloud;
				} else if (ec2endpoint.getHost().contains(
						EC2Constant.VMWARECLOUDNAME)) {
					LOGGER.info("Testing connection for VMware");

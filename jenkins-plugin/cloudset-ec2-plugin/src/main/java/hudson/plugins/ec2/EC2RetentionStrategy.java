...
public class EC2RetentionStrategy extends RetentionStrategy<EC2Computer> {
    ...
    public synchronized long check(EC2Computer c) {
    	...
    	if (StringUtils.contains(c.getNode().getLabelString(),
    		...
    		LOGGER.info("Get start time of Slave "+c.getConnectTime());
        ...
        }else if (StringUtils.contains(c.getNode().getLabelString(),
			...
			if (Jenkins.getInstance().getQueue().getBuildableItems(c).isEmpty() && c.isIdle() && !c.isConnecting() && c.getIdleStartMilliseconds()> 0
				...
				try {
					...
					LOGGER.info("Instance Id :: " + c.getNode().getInstanceId()
							...
							+ " User Id :: " + c.getNode().userId);
					LOGGER.info("c.getBuilds() details " + c.getBuilds());
					LOGGER.info("c.getBuilds() is empty details "
							+ c.getBuilds().isEmpty());
					LOGGER.info("c.isConnecting() is empty details "
							+ c.isConnecting());
					LOGGER.info("c.getIdleStartMilliseconds() "
							+ c.getIdleStartMilliseconds());
					LOGGER.info("buildable item "
							+ Jenkins.getInstance().getQueue()
									.getBuildableItems(c));
					LOGGER.info("number of busy executor " + c.countBusy());
					LOGGER.info("is idle " + c.isIdle());
					LOGGER.info("Get start time of Slave " + c.getConnectTime());
					LOGGER.info("--------------Get Up time ------------------" + c.getUptime());

					LOGGER.info("Get current time "
							+ System.currentTimeMillis());

					LOGGER.info("Get difference from connect time "
							+ (System.currentTimeMillis() - c.getConnectTime()));

					LOGGER.info("Get difference from Up time "
							+ (System.currentTimeMillis() - c.getUptime()));

					if (c.getNode().stopWhenBuildCompleted) {

						// Wait for 50 mins since the machine was started before
						// stopping the instance, Amazon will charge multiple times for minimum 1 hrs if the
						// machine was started multiple times
						if ((c.getUptime()) > 3000000 ) {
							c.getNode().stopInstance(
									c.getNode().getInstanceId(),
									c.getNode().getProjectUserID(),
									c.getNode().getProjectID(),
									new String[] { c.getNode().accountNumber,
